...
event(), clock(), collisionAndFriction(), DT(0.0f) {
	...
	gameObject.rectangle = std::make_shared<Rect>(1, 60.0f, sf::Vector2f(400.0f, 200.0f),
	...
	gameObject.path.push_back(std::make_shared<Path>(2, mass, sf::Vector2f(blockSize.x, 504.0f), blockSize, nil, nil,
		E, u));
	gameObject.path.push_back(std::make_shared<Path>(3, mass, sf::Vector2f(660.0f, 504.0f), blockSize, nil, nil,
		E, u));
	for (int i = 0; i < 11; ++i) {
		gameObject.path.push_back(std::make_shared<Path>(4 + i, mass, sf::Vector2f( blockSize.x * (i+1) , 550.0f),
...
void PhysicLoop::Load(){
	...
	gameObject.grid.AddObject(gameObject.rectangle);
	for (int i = 0; i < gameObject.path.size(); ++i) {
		gameObject.path[i]->Load();
		gameObject.grid.AddObject(gameObject.path[i]);
	}
}
...
void PhysicLoop::Update() {
	...
	/*if (size > 0) {
		...
		for (int i = 0; i < size; ++i) {
			std::cout << "collision with id =  " << potential[i]->GetIndex() << std::endl << std::endl;
		}
	}*/
	for (int i = 0; i < gameObject.path.size(); ++i) {
