...
GridResult GameGrid::FindUpdatableAndDrawableBlock(sf::Vector2f halfScreenPosition){

    GridResult result;
	sf::Vector2i range(GMNumber::SEARCH_RANGE_FOR_UPDATE_DRAW);
    sf::Vector2i objectGridNumber = GetGridNumber(halfScreenPosition);
...
void GameGrid::MoveObject(std::shared_ptr<GameShape> shape) {
    const sf::Vector2f& newPos = shape->GetPosition();
    const sf::Vector2f& oldPos = shape->GetOldPosition();
