...
void Level::Update(const float& dt) {
    ...
    if (!this->isPaused) {
        ...
        this->updateDrawResultFromGrid = this->grid.FindUpdatableAndDrawableBlock(viewCenter);
    ...
    , const float& coeffOfFriection){
        ...
        for (unsigned int i = 0; i < bouncyPathData.size(); ++i) {
            this->bouncyPath.push_back(std::make_shared<BouncyPath>(++this->entityIdCounter,
                ...
                this->tileSize, GMNumber::COEFF_OF_RESTITUTION_FOR_BOUNCY_PATH, coeffOfFriection));
        }
}


void Level::CreateCollectable(const TotalTileData& data){
    unsigned int CollectablePoints = 10;
	...
	for (unsigned int i = 0; i < collectableData.size(); ++i) {
		this->collectable.push_back(std::make_shared<Collectable>(++this->entityIdCounter,
			CollisionId::CollectableId, CollectablePoints, collectableData[i].textureIndex,
